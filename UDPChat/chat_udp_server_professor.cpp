...
void sendDatagram(
    ...
    string seq = padNumber(sequence, 4);
...
string buildProtocol(
    ...
    protocol += padNumber(nickname.size(), 3);
    protocol += nickname;
    ...
    protocol += padNumber(filename.size(), 11);
    protocol += filename;

    protocol += padNumber(fileData.size(), 20);
    protocol += fileData;
...
void parseProtocol(
    ...
    int nickSize = stoi(data.substr(idx, 3));
    idx += 3;

    nickname = data.substr(idx, nickSize);
    idx += nickSize;
    ...
    filename = data.substr(idx, fileNameSize);
    idx += fileNameSize;
...
int main() {
    ...
    while (true) {
        ...
        if (flag == "01") {
            ...
            fragmentBuffer[clientKey].push_back({
                sequence,
                payload
            });
        }
